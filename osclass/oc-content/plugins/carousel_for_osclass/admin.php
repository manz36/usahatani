<?php
    ...
    $random            = '';
    $dao_preference = new Preference();
    if(Params::getParam('random') != '') {
        $random = Params::getParam('random');
    } else {
    ...
    if( Params::getParam('option') == 'stepone' ) {
        ...
        $dao_preference->update(array("s_value" => $random), array("s_section" => "plugin-carousel_for_osclass", "s_name" => "carousel_random"));        
...
<form action="<?php osc_admin_base_url(true); ?>" method="post">
    ...
    <fieldset>
        ...
        <br />
        <label for="random" style="font-weight: bold;"><?php _e('Display ads in a random order?','carousel'); ?></label>:<br />
        <select name="random" id="random">
        	<option <?php if($random ==0){echo 'selected="selected"';}?> value='0'><?php _e('No', 'carousel_for_osclass'); ?></option>
        	<option <?php if($random ==1){echo 'selected="selected"';}?> value='1'><?php _e('Yes', 'carousel_for_osclass'); ?></option>
        </select>
        <br />
